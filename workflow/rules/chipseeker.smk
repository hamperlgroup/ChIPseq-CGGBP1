...
rule SLAMseq_overlap:
    ...
    output:
        "results/ChIP-peaks/{PEAKS}/SLAM-seq_overlap/overlap_{IP}-SLAM_bulk.tsv",
        "results/ChIP-peaks/{PEAKS}/SLAM-seq_overlap/overlap_{IP}-SLAM_nascent.tsv"
    params:
        sample = "{IP}",
        fc = 1,
        pval = 1,
    ...
    shell:
        """
            Rscript workflow/scripts/slam-seq_gene_overlap.R --sampleName {params.sample} --inputFile {input.peaks} --absCutoffFC {params.fc} --cutoffPval {params.pval} --peakMode {params.peakMode}
        """
