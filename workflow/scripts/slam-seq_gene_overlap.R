...
option_list = list(
  ...
  make_option(c("--absCutoffFC"), type="double",
              help="Absolute cutoff value to filter FC", metavar="FC"),
  make_option(c("--cutoffPval"), type="double",
...
PlottingScatter <- function(sample, rnaSet, rnaType){
  ...
  if(identical(empty, overlapping_genes)){
    ...
    plot <- ggplot(overlap, aes(x=log2FC, y=pvalue)) +
        geom_point() +
        geom_text(label=overlap$GeneID, 
    ...
    pdf(paste0(outDir,'/SLAM-',rnaType,'_',sample,'.pdf'))
      print(plot)
    dev.off()

    ## Save hits
    write.table(overlap, paste0(outDir,'/overlap_',sample,'-SLAM_',rnaType,'.tsv'), sep='\t', quote=F, row.names=F)
...
if (!dir.exists(paste0(roothPath,"/results/ChIP-peaks/",peakMode,"/SLAM-seq_overlap"))){
    dir.create(paste0(roothPath,"/results/ChIP-peaks/",peakMode,"/SLAM-seq_overlap"))}
